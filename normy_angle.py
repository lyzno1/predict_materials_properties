...
class StructureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target = self.target[idx].squeeze(-1)
...
class AttentionStructureModel(nn.Module):
    def __init__(self, embedding_dim=10, hidden_size=64, output_size=1, dropout=0.2, num_features=10):
        ...
        self.angle_expansion = AngleExpansion(num_features=num_features)
        self.angle_expansionrbf = AngleExpansionRBF(num_features=num_features, sigma=args.sigma)
...
class liu_attention_Lightning(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        targets = batch['target'].float()
        outputs = self(distances, angles)
        original_predict = self.pt.inverse_transform(outputs.cpu().numpy().reshape(-1,1))
        test_loss = F.l1_loss(torch.from_numpy(original_predict).to(device), targets.unsqueeze(-1))
...
def prepare_dataloader(train_inputs, train_outputs, max_length, batch_size, num_worker):
    ...
    train_outputs_reshaped = train_outputs.values.reshape(-1, 1)
    transformed_train_outputs = pt.fit_transform(train_outputs_reshaped)

    dataset = StructureDataset(distance_inputs, angle_inputs, transformed_train_outputs)
...
def visualize_results(results_list, mb_dataset_name): # 可视化结果并保存到文件中
    ...
    with open('add_angle.txt', 'a') as f:
        ...
        f.write(f"{mb_dataset_name}, batch_size:{args.batch_size}, Average MAE: {average_mae}\n")
        f.write('\n')
        f.write("-" * 100)
        f.write('\n')
    results_list.clear()
