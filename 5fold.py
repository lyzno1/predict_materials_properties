...
test_data_matbench_fold = []  # 创建一个空列表来存储测试数据

for i in range(5):
    test_data_matbench_fold.append(task.get_test_data(fold_number=i, include_target=True))

X, y = train_data_matbench
testX = []
testy = []
for i in range(5):
    X_i, y_i = test_data_matbench_fold[i]
    testX.append(X_i)
    testy.append(y_i)
...
for i in range(5):
    testX[i] = get_triplets(testX[i])
    testX[i] = torch.tensor(testX[i])
    testX[i] = testX[i].to(device)
    print(testX[i].size(1))
...
class MyGRUModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, dropout_prop, running_mean, test_running_mean):
        ...
        self.self_attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=1)
        self.self_attention1 = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=2)
    ...
    def forward(self, structures):
        ...
        if gru_output.size(1) == running_mean:
            gru_output = self.bn(gru_output)  # 在GRU输出后应用批归一化
        elif gru_output.size(1) == test_running_mean:
            gru_output = self.bn1(gru_output)
        else:
            gru_output = self.bn2(gru_output)


        # 调用 self-attention
        gru_output = gru_output.permute(1, 0, 2)  # 调整维度顺序以匹配 MultiheadAttention 的输入要求
        ...
        attention_output = attention_output.permute(1, 0, 2)  # 恢复原来的维度顺序
        ...
        return output
# attention 在embedding之后，cut out，
...
# 实例化模型
model = MyGRUModel(input_size, hidden_size, output_size, dropout_prop, running_mean, test_running_mean)
...
for fold in range(5):
    test_data.append(testX[fold])
    test_targets.append(torch.tensor(testy[fold].values, device=device).float())
    test_set.append(torch.utils.data.TensorDataset(test_data[fold], test_targets[fold]))
    test_loader.append(torch.utils.data.DataLoader(test_set[fold], batch_size=32, shuffle=False))
...
with torch.no_grad():
    for fold, loader in enumerate(test_loader):
        fold_loss = 0
        ...
        fold_loss /= len(loader)  # 计算每个fold的平均loss
        fold_losses.append(fold_loss)  # 将每个fold的loss存储起来
        total_test_loss += fold_loss

average_test_loss = total_test_loss / len(test_loader)  # 计算所有fold的平均loss

# 打印每个fold的loss
for fold, loss in enumerate(fold_losses):
    print(f'Fold {fold + 1} Test Loss: {loss:.4f}')

print(f'Average Test Loss Across Folds: {average_test_loss:.4f}')
