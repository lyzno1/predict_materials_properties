...
class Attention_structer_model(nn.Module):
    def __init__(self, embedding_dim=10, hidden_size=64, output_size=1, dropout=0.2, num_features=10, gamma=1.0):
...
class liu_attention_Lightning(pl.LightningModule):
    def __init__(self, hparams=None):
        ...
        self.model = Attention_structer_model(embedding_dim=hparams.embedding_dim,
...
def main(hparams):
    ...
    pl.seed_everything(init_seed, workers=True)
    ...
    for task in mb.tasks:
        ...
        for fold in task.folds:
            structures, properties = task.get_train_and_val_data(fold)

            max_length = TripletStats(structures).get_average() * hparams.cutoff  # 用于截断/补齐
            triplets = torch.tensor(get_triplets(structures, max_length))  # 处理输入

            dataset = StructureDataset(triplets, properties)
            train_data, val_data = train_test_split(dataset, test_size=0.2, random_state=init_seed)
            train_loader = DataLoader(train_data, batch_size=hparams.batch_size, num_workers=hparams.num_worker,
                                      shuffle=True, persistent_workers=True)
            val_loader = DataLoader(val_data, batch_size=hparams.batch_size, num_workers=hparams.num_worker,
                                    shuffle=True, persistent_workers=True)

            lightning_model = liu_attention_Lightning(hparams=hparams)
            ...
            lightning_best_model = liu_attention_Lightning.load_from_checkpoint(best_model_path,
            ...
            predict = trainer.test(model=lightning_best_model, dataloaders=test_loader)
