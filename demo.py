...
class MyGRUModel(nn.Module):
    ...
    def forward(self, structures):
        ...
        for triplet_data in structures:
            for only_data in triplet_data:
                if only_data[0] not in embedded_indices:
                    if only_data[0] != 0:
                        embedded_indices[only_data[0]]=self.embedding(only_data[0].to(torch.long).to(self.device))

                if only_data[1] not in embedded_indices:
                    if only_data[1] != 0:
                        embedded_indices[only_data[1]]=self.embedding(only_data[1].to(torch.long).to(self.device))
        print(embedded_indices)

        # embedded_indices = {}
        # for atomic_index in embedded_indices:
        #     if atomic_index != 0:
        #         embedded_indices[atomic_index] = self.embedding(atomic_index.to(torch.long).to(self.device))

        for triplet_data in structures:
            embedded_triplet_data = []

            for only_data in triplet_data:
                embedding_i = embedded_indices.get(only_data[0].to(self.device), zero_tensor)
                embedding_j = embedded_indices.get(only_data[1].to(self.device), zero_tensor)
                print(embedding_i)
                if torch.all(embedding_i == zero_tensor):
                    distance_tensor = zero_tensor
                else:
